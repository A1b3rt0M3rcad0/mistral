...
Character-scoped routes are behind a provider-neutral `PrincipalResolver` plus ownership authorization. The default binary intentionally does not install an authentication provider yet, so protected routes fail closed until a concrete authentication adapter is selected.
